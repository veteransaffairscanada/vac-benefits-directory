...
#### Why/how are we using it?

We're using AirTable because it allows non-developers to edit the data used by our app. This is very useful for:
...
2. Eligibility logic: The logic used by our app to determine who is eligible for what benefits is set in the eligibilityPaths table. The questions we ask to determine who a user is each have 1 column: patronType, serviceType, statusAndVitals, and serviceHealth issue. Each row has a unique combination of answers to these questions, and a list of benefits that are likely available to a user who answers in this way. The list of benefits are linked to rows in the benefits table.
...
To request access to edit content in AirTable, set up an account on their website and then [email us](email:vac@cds-snc.ca) with your account details so we can add you to the project.
